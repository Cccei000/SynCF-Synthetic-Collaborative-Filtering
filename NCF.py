...
def parse_args():
    ...
    parser.add_argument('--mlplayers',nargs = '?',default = '[64,16]',
...
def get_model(train, num_users, num_items, userlayers=[512, 64],
              itemlayers=[1024, 64], mlplayers = [64,16]):
    ...
    # NCF representation learning part
    ...
    # MLP prediction part
    predict_vector = concatenate([user_latent_vector, item_latent_vector])
    print(predict_vector.shape)
    
    for idx in range(len(mlplayers)):
        predict_vector = Dense(mlplayers[idx],activation = 'relu',
...
if __name__ == '__main__':
    ...
    model_out_file = 'Pretrain/%s_NCF_%d.h5' %(args.dataset, time())
    ...
    # Build model
    model = get_model(train, num_users, num_items, userlayers, itemlayers, mlplayers)
    print(model.summary())
    ...
    if args.out > 0:
        print("The best NCF model is saved to %s" % model_out_file)
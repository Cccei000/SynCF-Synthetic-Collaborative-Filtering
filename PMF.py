...
def parse_args():
    parser = argparse.ArgumentParser(description="Run PMF.")
    ...
    parser.add_argument('--batch_size', type=int, default=256,
                        help='Batch size.')
    parser.add_argument('--userlayers', nargs='?', default='[512, 64, 16]',
                        help="Size of each user layer")
    parser.add_argument('--itemlayers', nargs='?', default='[1024, 64, 16]',
                        help="Size of each item layer")
    parser.add_argument('--predictionlayers',nargs = '?',default = '[64,16]',
                        help = 'Size of each prediction layer')
...
def get_model(train, num_users, num_items, userlayers=[512, 64, 16],
              itemlayers=[1024, 64, 16], predictionlayers = [64,16]):
    ...
    # PMF representation learning part
    ...
    # Outer product layer
    _slice = Lambda(getslice,arguments = {'index':0})(item_latent_vector)
    predict_vector = multiply([user_latent_vector,_slice])
    print(_slice.shape,predict_vector.shape)
    for idx in range(item_latent_vector.shape[1]-1):
        _slice = Lambda(getslice,arguments = {'index':idx+1})(item_latent_vector)
        _slice = multiply([user_latent_vector,_slice])
        predict_vector = concatenate([predict_vector,_slice])
        print(predict_vector.shape)

    # MLP prediction part
    for idx in range(len(predictionlayers)):
        predict_vector = Dense(predictionlayers[idx],activation = 'relu',
                               name = 'prediction%d'%idx)(predict_vector)
        print(predict_vector.shape)
    
    # Final prediction layer
    prediction = Dense(1,activation = 'sigmoid',kernel_initializer = initializers.lecun_normal(),
                       name = 'predicion')(predict_vector)
...
if __name__ == '__main__':
    ...
    predictionlayers = eval(args.predictionlayers)
    ...
    print("PMF arguments: %s " %(args))
    model_out_file = 'Pretrain/%s_PMF_%d.h5' %(args.dataset, time())
    ...
    # Build model
    model = get_model(train, num_users, num_items, userlayers, itemlayers, predictionlayers)
    print(model.summary())
    ...
    if args.out > 0:
        print("The best PMF model is saved to %s" % model_out_file)